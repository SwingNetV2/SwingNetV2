...
# 원본 채널 파라미터
xyx = 13  # conv3d output channels
abc = 19  # final 2D conv channels per branch
...
class EventDetector_clstm_lr(nn.Module):
    def __init__(self, n_conv=64, num_classes=9):
        ...
        self.shared_conv = nn.Sequential(
            ...
            nn.Conv2d(30, abc, kernel_size=3, padding=1)
        )
        
        # 3D convolution
        self.conv3d = nn.Conv3d(abc, xyx, kernel_size=3, padding=1)
        self.batchnorm = nn.BatchNorm3d(xyx, momentum=0.8)
        
        # Bidirectional GRU
        self.gru1 = nn.GRU(xyx * 40 * 40, 256, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):
        ...
        # 단일 forward pass로 모든 프레임 처리 (기존 대비 64배 빠름)
