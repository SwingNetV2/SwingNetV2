...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--path', help='Path to video that you want to test', default='output/test_video2.mp4')
    ...
    # try:
    #     save_dict = torch.load('model/clstm_model.pth')
    # except:
    ...
    for i, e in enumerate(events):
        ...
        img_pil.save(f'output2/image{e}.jpg')
        cv2.putText(img, '{} , {}'.format(event_names[i], format(e)), (20, 50), cv2.FONT_HERSHEY_DUPLEX, 0.75, (0, 0, 255), 2)
        cv2.imshow(img)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        image = mp.Image.create_from_file(f'output2/image{e}.jpg')
        ...
        if landmarks:
            ...
            pil_image.save(f'output2/output{e}.jpg') # Save with event frame number
            cv2.imshow(image_np)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
